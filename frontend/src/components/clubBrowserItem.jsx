...
export default function ClubBrowserItem({ club, memberCount, loggedIn, clubStatus }) {
  return (
    ...
    <Link to={"/clubpage/" + club.name}>
      <div className="relative flex flex-row overflow-hidden bg-secondary h-28 gap-6 p-2 my-0.5 border-3 border-primary rounded-md items-center transition duration-150 ease-in-out hover:brightness-90 hover:-translate-y-1 hover:scale-102">
        ...
        {/* Add club button */}
        {loggedIn && (
          <FaPlus className="z-10 bg-primary text-secondary size-12 p-1.5 rounded-md ml-auto mr-2 transition ease-in-out hover:brightness-75 hover:scale-125"></FaPlus>
        )}
      </div>
    </Link>
  );
